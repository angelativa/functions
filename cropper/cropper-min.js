(function (global, factory) {
  ...
  function drawDashedLine(ctx, startX, startY, endX, endY, dashLength) {
    ...
    for (var i = 0; i < numDashes; ++i) {
      ctx[i % 2 === 0 ? 'moveTo' : 'lineTo'](startX + deltaX / numDashes * i, startY + deltaY / numDashes * i);
    }
  ...
  Cropper.prototype = {
    ...
    getCropperImage: function () {
      ...
      for (var i = 0; i < sizesList.length; i++) {
        ...
        ctx.drawImage(me.image, (rect.startX - me.sourceImage.sx) / me.sourceImage.sw * me.image.width, (rect.startY - me.sourceImage.sy) / me.sourceImage.sh * me.image.height, rect.width / me.sourceImage.sw * me.image.width, rect.height / me.sourceImage.sh * me.image.height, 0, 0, canvas.width, canvas.height);
    ...
    isPointInCropperBox(x, y) {
      ...
      if (rect.startX < x && x < rect.startX + rect.width && rect.startY < y && y < rect.startY + rect.height) {
        return true;
      }
    ...
    resizeBox: function (offsetX, offsetY, offsetWidth, offsetHeight) {
      ...
      startX = startX + boxWidth > sourceImage.sx + sourceImage.sw ? sourceImage.sx + sourceImage.sw - boxWidth : startX;
      startY = startY + boxHeight > sourceImage.sy + sourceImage.sh ? sourceImage.sy + sourceImage.sh - boxHeight : startY;
    ...
    bindEvent: function () {
      ...
      me.canvas.onmousemove = function (event) {
        ...
        if (Math.abs(points[3].x - x) < CHECKED_AREA_SIZE && Math.abs(points[3].y - y) < CHECKED_AREA_SIZE) {
          me.cursorAction = 'se-resize';
        } else if (Math.abs(points[2].x - x) < CHECKED_AREA_SIZE && Math.abs(points[2].y - y) < CHECKED_AREA_SIZE) {
          me.cursorAction = 'sw-resize';
        } else if (Math.abs(points[1].x - x) < CHECKED_AREA_SIZE && Math.abs(points[1].y - y) < CHECKED_AREA_SIZE) {
          me.cursorAction = 'ne-resize';
        } else if (Math.abs(points[0].x - x) < CHECKED_AREA_SIZE && Math.abs(points[0].y - y) < CHECKED_AREA_SIZE) {
    ...
    drawCropperRect: function (x, y) {
      ...
      var points = me.boxRect.points; // 绘制图
      ...
      drawLine(ctx, startX, startY + boxHeight, startX, startY); // 绘制九宫格
      ...
      drawDashedLine(ctx, startX + boxWidth / 3 * 2, startY, startX + boxWidth / 3 * 2, startY + boxHeight); // 绘制 4 个点
    ...
    drawSourceImage: function () {
      ...
      if (sourceWidth < me.canvasWidth && sourceHeight < me.canvasHeight) {
      ...
      me.ctx.drawImage(me.image, 0, 0, me.image.width, me.image.height, startX, startY, sourceWidth, sourceHeight);
      me.sourceImage = {
        sx: startX,
        sy: startY,
        sw: sourceWidth,
